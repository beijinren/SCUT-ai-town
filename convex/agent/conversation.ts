...
export async function startConversationMessage(
  ...
  if (extraSystemPrompt) {
    /*
     * This hidden constraint is only for the current retry.
     * It should never appear as an in-world message.
     */
    prompt.push(extraSystemPrompt);
  }
...
export async function continueConversationMessage(
  ...
  const llmMessages: LLMMessage[] = [
    {
      role: 'system',
      content: extraSystemPrompt ? `${prompt.join('\n')}\n${extraSystemPrompt}` : prompt.join('\n'),
    },
