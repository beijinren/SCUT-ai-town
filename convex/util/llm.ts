...
function getChatConfig(): LLMEndpointConfig {
  const apiKey = process.env.CHAT_API_KEY ?? process.env[DEFAULT_CHAT_API_KEY_ENV];
  if (!apiKey) {
    throw new Error(
      `Chat model is forced to ${DEFAULT_CHAT_MODEL}. Set CHAT_API_KEY or ${DEFAULT_CHAT_API_KEY_ENV}.`,
    );
  }

  // Chat generation is intentionally locked to the project-level DeepSeek flash model.
  // Keep CHAT_API_URL configurable only for OpenAI-compatible gateways or local proxies.
  return {
    provider: DEFAULT_CHAT_PROVIDER,
    url: process.env.CHAT_API_URL ?? DEFAULT_CHAT_API_URL,
...
export async function chatCompletion(
  ...
  // Hard override: callers cannot accidentally use another chat model.
