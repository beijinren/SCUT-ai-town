...
function getLegacyLLMConfig(): LegacyLLMConfig {
  ...
  if (process.env.TOGETHER_API_KEY) {
    ...
    return {
      ...
      chatModel:
        process.env.TOGETHER_CHAT_MODEL ?? "meta-llama/Llama-3-8b-chat-hf",
      embeddingModel:
...
function getChatConfig(): LLMEndpointConfig {
  if (hasSplitChatConfig()) {
    ...
    return {
      provider:
        (process.env.CHAT_PROVIDER as LLMProvider | undefined) ?? "custom",
      url,
      model,
...
function getEmbeddingConfig(): LLMEndpointConfig {
  if (hasSplitEmbeddingConfig()) {
    const provider =
      (process.env.EMBEDDING_PROVIDER as LLMProvider | undefined) ?? "openai";
    ...
    if (provider === "together") {
      const apiKey =
        process.env.EMBEDDING_API_KEY ?? process.env.TOGETHER_API_KEY;
      if (!apiKey) {
        throw new Error(
...
function llmConfigFromProfileState(state: ConnectionProfileState): LLMConfig {
  const activeProfile =
    state.profiles.find(
      (profile) => profile.name === state.activeProfileName,
    ) ?? state.profiles[0];
  if (!activeProfile) {
    return getLLMConfig();
  }
...
export async function chatCompletion(
  ...
  const body = (maybeBody ?? ctxOrBody) as Omit<
...
export async function fetchEmbedding(
  ...
  const { embeddings, ...stats } = ctx
    ? await fetchEmbeddingBatch(ctx, [text])
    : await fetchEmbeddingBatch([text]);
