import type { WorldMemorySnapshot, MemorySnapshotEntry } from '../../convex/agent/memory.ts';
...
function characterMemoryFileName(character: string, name: string, playerId: string) {
...
export function buildCharacterMemoryDocument(
  ...
  return {
    ...
    conversationExperiences: [...snapshot.memories.conversation].sort(memorySort),
  };
}

export function buildMemoryFiles(snapshot: WorldMemorySnapshot): MemoryFileEntry[] {
  const generatedAt = snapshot.updatedAt;
  const files = snapshot.characters.map((character) => {
    const document = buildCharacterMemoryDocument(character, String(snapshot.worldId), generatedAt);
    const fileName = characterMemoryFileName(character.character, character.name, character.playerId);
    return {
      filePath: fileName,
  ...
  const indexDocument = {
    ...
    characters: snapshot.characters.map((character) => ({
      ...
      filePath: characterMemoryFileName(character.character, character.name, character.playerId),
      counts: {
