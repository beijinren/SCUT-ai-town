...
export function SceneInfoPanel({ worldId }: { worldId: Id<'worlds'> }) {
  ...
  const infoPayload = useQuery(worldApi.sceneDebugViews, { worldId }) as SceneInfoPayload | undefined;
  ...
  if (!infoPayload) {
    return (
      <div className="mt-6 desc">
        <p className="leading-tight -m-4 bg-brown-700 text-base sm:text-sm">正在加载场景信息...</p>
      </div>
    );
  }
