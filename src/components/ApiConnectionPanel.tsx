...
type StatusKind =
  | TestState["status"]
  | ModelFetchState["status"]
  | MessageTestState["status"];
...
export function ApiConnectionPanel({
  ...
  const backendState = useQuery(
    api.agent.connectionProfiles.getConnectionState,
  ) as StoredState | null | undefined;
  const saveConnectionState = useMutation(
    api.agent.connectionProfiles.saveConnectionState,
  );
  ...
  const [activeProfileName, setActiveProfileName] = useState<string>(
    initialState.activeProfileName,
  );
  const [workingConfig, setWorkingConfig] = useState<ApiConnectionConfig>(
    initialState.profiles.find(
      (profile) => profile.name === initialState.activeProfileName,
    )?.config ?? initialState.profiles[0].config,
  );
  ...
  useEffect(() => {
    ...
    const activeProfile =
      backendState.profiles.find(
        (profile) => profile.name === backendState.activeProfileName,
  ...
  const unsaved =
    !!activeProfile &&
    JSON.stringify(activeProfile.config) !== JSON.stringify(workingConfig);
  ...
  function updateWorkingConfig(
    updater: (prev: ApiConnectionConfig) => ApiConnectionConfig,
  ) {
  ...
  async function testConnection() {
    ...
    try {
      ...
      setTestState({
        status: "ok",
        message: "OpenAI-compatible 接口连接成功。",
      });
    } catch (error) {
  ...
  async function fetchAvailableModels() {
    ...
    try {
      ...
      setModelFetchState({
        status: "ok",
        message: `已拉取 ${modelNames.length} 个模型。`,
      });
    } catch (error) {
      const detail = error instanceof Error ? error.message : "未知错误";
      setAvailableModels([]);
  ...
  async function sendTestMessage() {
    ...
    try {
      ...
      const text =
        data.choices?.[0]?.message?.content ?? "收到响应，但内容为空。";
      setMessageTestState({
        status: "ok",
        message: "测试消息发送成功。",
        response: text,
      ...
      <div className="desc mt-4">
        <div className="leading-tight -m-4 bg-brown-700 text-base sm:text-sm p-4 space-y-3">
          ...
          <div className="flex gap-2">
            ...
            />
            <button
              className="rounded bg-clay-700 px-3 py-1"
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <button
              className="rounded bg-clay-700 px-3 py-1"
              onClick={reloadCurrentProfile}
            >
              重载
            </button>
            <button
              className="rounded bg-rose-800 px-3 py-1"
              onClick={deleteCurrentProfile}
            >
              删除
            </button>
            <button
              className="rounded bg-amber-700 px-3 py-1"
              onClick={() => setShowInfo((v) => !v)}
            >
              信息
            </button>
            <button
              className="rounded bg-brown-500 px-3 py-1"
          ...
          <div className="flex items-center justify-between gap-3">
            <span
              className={`inline-block rounded px-2 py-0.5 text-xs ${statusClass(testState.status)}`}
            >
              {testState.status.toUpperCase()}
            </span>
            <span className="text-brown-200">
              {unsaved ? "有未保存修改" : "配置已保存"}
            </span>
          </div>
          ...
          {showInfo && activeProfile && (
            <div className="rounded border border-brown-500 p-2">
              ...
              <p>
                当前设计：档案会同时保存到浏览器本地与 Convex，切换后立刻生效。
              </p>
            </div>
          )}
        </div>
      ...
      <div className="desc mt-4">
        <div className="leading-tight -m-4 bg-brown-700 text-base sm:text-sm p-4 space-y-3">
          ...
          {roleLocatorEntries.map((role) => (
            <details
              key={role.playerId}
              className="rounded border border-brown-500 p-2"
            >
              ...
              <div className="mt-2 space-y-1 text-brown-100">
                ...
                {role.publicProfile && (
                  <p>publicProfile：{role.publicProfile}</p>
                )}
              </div>
            </details>
          ))}
